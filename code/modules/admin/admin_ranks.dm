...
/hook/startup/proc/loadAdmins()
	load_admins()
	load_shunned() // DARKHOLM'S DUNGEON BAYBEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
	return 1
...
// DARKHOLM'S DUNGEON ADDITION: attempt to create the SHUNNED LIST
/proc/load_shunned()
	//clear the datums references
	shunned_datums.Cut()
	//log_admin("does this function even run?")
	//message_admins("does this function even run???", 1)
	/*for(var/client/C in admins)
		C.remove_admin_verbs()
		C.holder = null
	admins.Cut()*/
	...
	for(var/line in shunLines)
		...
		//create the admin datum and store it for later use
		var/datum/shunned/D = new /datum/shunned(ckey)

		//find the client for a ckey if they are connected and associate them with the new admin datum
		D.associate(directory[ckey])
		log_admin("[ckey] is SHUNNED WOAHWOAHWOAHWOAHWOAHWO")
		message_admins("[ckey] is a SHUNNED ASS NIGGA", 1)
		shunned_list += ckey
