...
/obj/structure/synthesized_instrument/synthesizer
	...
	show_menu_for(mob/user)
		...
		dat += "Virtual environment: [musical_all_environments[player.virtual_environment_selected+2]]"
		dat += "<A href='?src=\ref[src];change_environment=1'>Change</A><BR>"
		...
		if(!edit)
			...
			if (coding_help)
				...
				dat += "Each condition can be enclosed in parenthesis<br>"
	...
	shouldStopPlaying(mob/user)
		if (src)
			if (player.song.autorepeat)
				return 0
			if(!src.Adjacent(user) || user.stat)
				return 1
			if (!anchored)
				return 1
			return 0
